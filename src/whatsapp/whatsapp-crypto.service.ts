...
export class WhatsAppCryptoService {
  ...
  constructor(config: ConfigService) {
    const encoded = config.get<string>('whatsapp.tokenEncryptionKey');
    if (!encoded) {
      this.key = undefined;
      return;
    }
    this.key = Buffer.from(encoded, 'hex');
    if (this.key.length !== 32) throw new Error('WHATSAPP_TOKEN_ENCRYPTION_KEY must be 32 bytes in hex');
  }
  ...
  decrypt(payload: string): string {
    try {
      ...
      throw new InternalServerErrorException('WhatsApp secure state is unavailable');
    }
  }

  private requiredKey(): Buffer {
    if (!this.key) throw new ServiceUnavailableException('WhatsApp integratsiyasi hozir sozlanmagan');
    return this.key;
  }
}
