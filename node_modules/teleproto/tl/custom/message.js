...
class CustomMessage extends senderGetter_1.SenderGetter {
    ...
    get toId() {
        ...
        if (this._client && !this.out && this.isPrivate) {
            return new api_1.Api.PeerUser({
                userId: (_a = this._client._selfInputPeer) === null || _a === void 0 ? void 0 : _a.userId,
            });
        }
        return this.peerId;
    }
    ...
    async react(reaction, big) {
        if (!this._client)
            return;
        function toReaction(r) {
            if (typeof r === "string") {
                return new api_1.Api.ReactionEmoji({ emoticon: r });
            }
            if (r instanceof api_1.Api.ReactionEmoji ||
                r instanceof api_1.Api.ReactionCustomEmoji ||
                r instanceof api_1.Api.ReactionPaid ||
                r instanceof api_1.Api.ReactionEmpty) {
                return r;
            }
            // BigInteger
            return new api_1.Api.ReactionCustomEmoji({
                documentId: (0, Helpers_1.returnBigInt)(r),
            });
        }
        let reactionList;
        ...
        return this._client.sendReaction((await this.getInputChat()), this.id, reactionList, big);
    }
    async getReactions(limit, reaction) {
        if (!this._client)
            return;
        return this._client.getReactionUsers((await this.getInputChat()), this.id, { limit, reaction });
    }
    async copy(entity) {
        if (!this._client)
            return;
        entity = await this._client.getInputEntity(entity);
        const request = new api_1.Api.messages.ForwardMessages({
            fromPeer: (await this.getInputChat()),
            id: [this.id],
            toPeer: entity,
            dropAuthor: true,
            randomId: [(0, big_integer_1.default)(Math.floor(Math.random() * 1e15))],
        });
