...
{ message, replyTo, attributes, parseMode, formattingEntities, linkPreview = true, file, thumb, forceDocument, clearDraft, buttons, silent, supportStreaming, schedule, noforwards, commentTo, topMsgId, sendAs, effect, invertMedia, } = {}) {
    if (file) {
...
/** @hidden */
async function forwardMessages(client, entity, { messages, fromPeer, silent, schedule, noforwards, dropAuthor, topMsgId, sendAs, effect, dropMediaCaptions, withMyScore, }) {
...
/** @hidden */
async function editMessage(client, entity, { message, text, parseMode, formattingEntities, linkPreview = true, file, forceDocument, buttons, schedule, invertMedia, }) {
...
async function deleteMessages(client, entity, messageIds, { revoke = false }) {
    ...
    if (ty == Helpers_1._EntityType.CHANNEL) {
        for (const chunk of utils.chunks(ids)) {
            results.push(client.api.channels.deleteMessages({
                channel: entity,
                id: chunk,
            }));
        }
        ...
        for (const chunk of utils.chunks(ids)) {
            results.push(client.api.messages.deleteMessages({
                id: chunk,
                revoke: revoke,
            }));
        }
    ...
    if (maxIdIsUndefined) {
        if (message) {
            if (Array.isArray(message)) {
                maxId = Math.max(...message.map((v) => utils.getMessageId(v)));
            }
            else {
                maxId = utils.getMessageId(message);
            }
        }
    }
    entity = await client.getInputEntity(entity);
...
// TODO do the rest
/** @hidden */
async function sendReaction(client, entity, messageId, reaction, big) {
...
async function getReactionUsers(client, entity, messageId, params) {
    ...
    return client.invoke(new tl_1.Api.messages.GetMessageReactionsList({
        ...
        offset: offset,
        reaction: reaction
            ? new tl_1.Api.ReactionEmoji({ emoticon: reaction })
            : undefined,
    }));
}
