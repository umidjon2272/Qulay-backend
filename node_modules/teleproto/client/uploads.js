...
async function uploadFile(client, fileParams) {
    ...
    let readAhead = Math.max(1, ul.maxSessions * Math.ceil(ul.maxWindow / Math.max(1, partSize)));
    ...
    const inflight = new OrderedWriter_1.BoundedSemaphore(Math.max(1, readAhead));
    let firstError;
    const tasks = [];
    for (let j = 0; j < partCount; j++) {
        ...
        tasks.push((async () => {
            try {
                ...
                await client._media.savePart(dcId, isLarge
...
/** @hidden */
async function _fileToMedia(client, { file, forceDocument, fileSize, progressCallback, attributes, thumb, voiceNote = false, videoNote = false, supportsStreaming = false, mimeType, asImage, workers, }) {
    if (!file) {
...
/** @hidden */
async function _sendAlbum(client, entity, { file, caption, formattingEntities, forceDocument = false, fileSize, clearDraft = false, progressCallback, replyTo, attributes, thumb, parseMode, voiceNote = false, videoNote = false, silent, supportsStreaming = false, scheduleDate, workers, noforwards, commentTo, topMsgId, sendAs, effect, invertMedia, }) {
