...
function decBlock(dk, s0, s1, s2, s3, out) {
    s0 = (s0 ^ dk[0]) >>> 0;
    s1 = (s1 ^ dk[1]) >>> 0;
    s2 = (s2 ^ dk[2]) >>> 0;
    s3 = (s3 ^ dk[3]) >>> 0;
    ...
    for (let r = 1; r < AES256_ROUNDS; r++) {
        const t0 = (Td0[(s0 >>> 24) & 255] ^
            Td1[(s3 >>> 16) & 255] ^
            Td2[(s2 >>> 8) & 255] ^
            Td3[s1 & 255] ^
            dk[k++]) >>>
            0;
        const t1 = (Td0[(s1 >>> 24) & 255] ^
            Td1[(s0 >>> 16) & 255] ^
            Td2[(s3 >>> 8) & 255] ^
            Td3[s2 & 255] ^
            dk[k++]) >>>
            0;
        const t2 = (Td0[(s2 >>> 24) & 255] ^
            Td1[(s1 >>> 16) & 255] ^
            Td2[(s0 >>> 8) & 255] ^
            Td3[s3 & 255] ^
            dk[k++]) >>>
            0;
        const t3 = (Td0[(s3 >>> 24) & 255] ^
            Td1[(s2 >>> 16) & 255] ^
            Td2[(s1 >>> 8) & 255] ^
            Td3[s0 & 255] ^
            dk[k++]) >>>
...
class IGE {
    ...
    xorBlock(dst, di, a, ai, b, bi) {
        for (let j = 0; j < 16; j++)
            dst[di + j] = a[ai + j] ^ b[bi + j];
    }
    encryptIge(plainText) {
        ...
        for (let j = 0, o = 0; o < n; j++, o += blockSize) {
            if (j === 0)
                plainText.copy(m, 0, 0, blockSize);
            else if (j === 1)
                this.xorBlock(m, blockSize, plainText, blockSize, iv2, 0);
            else
                this.xorBlock(m, o, plainText, o, plainText, o - 2 * blockSize);
        }
        const cbc = node_crypto_1.default.createCipheriv("aes-256-cbc", this.key, iv1);
        cbc.setAutoPadding(false);
        const d = Buffer.concat([cbc.update(m), cbc.final()]);
        const output = Buffer.allocUnsafe(n);
        for (let j = 0, o = 0; o < n; j++, o += blockSize) {
            if (j === 0)
                this.xorBlock(output, 0, d, 0, iv2, 0);
            else
                this.xorBlock(output, o, d, o, plainText, o - blockSize);
        }
        return output;
    }
    decryptIge(cipherText) {
        ...
        const output = Buffer.allocUnsafe(n);
        const cv = new DataView(cipherText.buffer, cipherText.byteOffset, n);
        ...
        for (let i = 0; i < n; i += blockSize) {
            ...
            decBlock(dk, (c0 ^ pp0) >>> 0, (c1 ^ pp1) >>> 0, (c2 ^ pp2) >>> 0, (c3 ^ pp3) >>> 0, tmp);
