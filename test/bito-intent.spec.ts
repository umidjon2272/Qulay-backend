...
describe('Bito intent boundaries', () => {
  ...
  it('extracts a concrete product search without turning generic stock asks into searches', () => {
    ...
    expect(bitoInventorySearchTerm('Sizlarda iPhone 13 Pro bormi?')).toBe('iphone 13 pro');
    expect(bitoInventorySearchTerm('Ayfon borm?')).toBe('iphone');
    expect(bitoInventorySearchTerm('omborn kurch aayfon bormikan iphone?')).toBe('iphone');
    expect(bitoInventorySearchTerm('iphone 13 por bormi?')).toBe('iphone 13 pro');
    expect(bitoInventorySearchTerm('Coca Cola 20 ta kerak')).toBe('coca cola');
    expect(bitoInventorySearchTerm('Coca Cola narxi qancha? 20 ta kerak')).toBe('coca cola');
    expect(bitoInventorySearchTerm('Cola 1.5 litr kerak')).toBe('cola');
